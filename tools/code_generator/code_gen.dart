...
String fromMap_jsonFieldToDart(Map<String, dynamic> varinfo, {String? varName}) {
  ...
  if (varinfo['type'] == 'Optional') {
    return fromMap_jsonFieldToDart(varinfo['optional_inner'], varName: varName);
  }
...
void createClass(File file, Map<String, dynamic> type, {String? parentClass}) {
  ...
  if (parentClass != null) {
    appendFile(file, 'late String? _type;\n');
  ...
  for (final field in type['struct_fields']) {
    ...
    appendFile(file, 'late $stype? _${field['name']};\n');
  ...
  if (type['struct_fields'].length > 0) {
    ...
    for (final field in type['struct_fields']) {
      final stype = jsonFieldTypeToDartType(field);
      var require = true;
      if (field['type'] == 'Optional') require = false;
      if(require) {
        appendFile(file, 'required $stype ${field['name']},');
      } else {
         appendFile(file, '$stype? ${field['name']},');
      }
  ...
  for (final el in module['types']) {
    if (el['name'] == pasteFromClass) {
      if (el['type'] == 'Struct') {
        ...
        paste['name'] = typeName;
        createClass(file, paste, parentClass: parentname);
      } else {
...
void createEnumsOfType(file, type, module) {
  ...
  for (final subtype in type['enum_types']) {
    if (subtype['type'] == 'Struct') {
      createClass(file, subtype, parentClass: type['name']);
...
void createEnumOfConsts(file, type) {
  ...
  appendFile(file, 'late String _value;\n');
